...
def permutation_feature_importance(
    ...
    """
    Calcola la Permutation Feature Importance per un modello PyTorch.
    """
    model.eval()
...
def train_and_evaluate_kfold_val_earlystop(
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(patient_ids, patient_labels)):
        ...
        plt.title(f"Train loss vs Val loss (fold:{fold+1})")
...
def prepare_sepsisexp_data(
    ...
    # features_selected = ['id', 'age', 'heart_rate', 'svri', 'mean_bp', 'svri', 
    # 'heart_time_volume', 'oxygen_saturation', 'delta-temperature',   
    # 'mixed_venous_oxygen_saturation', 'lactate', 'creatinine', 'bilirubin', 'sodium', 'potassium',
    # 'hemoglobin', 'chloride', 'leukocytes', 'bicarbonate', 'pancreatic_lipase', 'blood_urea_nitrogen',
    # 'procalcitonin', 'alanine_transaminase', 'bun/creatinine_ratio', 'aspartate_transaminase', 'c-reactive_protein', 'respiratory_minute_volume',
    # 'fraction_of_inspired_o2', 'arterial_ph', 'partial_pressure_art._o2', 'sepsis', 'timestep', 'severity']

    features_selected = merged_df.columns

    hour_df=hour_df[features_selected]
...
def prepare_mimic_data(mimic_path='./datasets/MIMIC/carry_forward/mean', target_len=24, pred_length=1, pop_patient=False, n=1, random_state=42, verbose=True):
    ...
    # mimic_df = mimic_df[[
    #     'Index',
    #     'meanbp', 'ph_bloodgas', 'chloride', 'resprate', 'heartrate', 'lactate', 'potassium', 'creatinine', 'fio2', 'tempc', 'pco2_bloodgas', 'ptt', 
    #     'spo2_pulsoxy', 'po2_bloodgas', 'o2flow', 'sodium', 'magnesium', 'calcium_free', 'hemoglobin', 'bilirubin', 'bicarbonate', 'glucose', 
    #     'so2_bloodgas', 'inr', 'ldh', 'platelet', 'creatinekinase', 'ck_mb', 'wbc',
    #     'timestep',
    #     'sepsis'
    # ]]
    ...
    original_features = ['sysbp', 'ph_bloodgas', 'diabp', 'bun', 'resprate', 'po2_bloodgas', 'chloride', 'fio2', 'lactate', 'pt', 'bicarbonate', 
    'meanbp', 'hematocrit', 'heartrate', 'inr', 'hemoglobin', 'o2flow', 'potassium', 'magnesium', 'so2_bloodgas', 'platelet', 'bilirubin', 
    'totalpeeplevel', 'wbc', 'calcium_free', 'pco2_bloodgas', 'creatinekinase', 'Index', 'sepsis', 'timestep'] # features selected
...
def train_final_model(
    ...
    print("\nApplying class balancing on training data only...")
    ...
    dummy_input = torch.randn(1, 24, 43).to(device)

    # Calcola l'output (necessario per costruire il grafo dei gradienti)
    output = model(dummy_input)

    # Crea l'oggetto Digraph
    dot = make_dot(output, params=dict(model.named_parameters()))

    # Salva l'immagine
    # cleanup=True rimuove il file sorgente .gv temporaneo, lasciando solo il PNG
    dot.render("architettura_modello", format="png", cleanup=True)

    print("Immagine salvata come: architettura_modello.png")

    criterion = loss_fn or nn.BCEWithLogitsLoss()
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay_value)
...
if __name__=="__main__":
    ...
    target_len = 24
    pred_length = 6
    # method="sequence_dtw_smote"
    method=None

    if dataset=="sepsisexp":
        data_by_patient, device, hour_df, _ = prepare_sepsisexp_data(target_len=target_len, pred_length=pred_length, pop_patient=False, n=10,
                                                                     random_state=42)
        feature_names = hour_df.drop(columns=["id", "sepsis", "timestep", "severity"]).columns.to_list()
    
    elif dataset=="mimic":
        data_by_patient, device, mimic_df, _, _, _= prepare_mimic_data(target_len=target_len, pred_length=pred_length, pop_patient=False, n=50, 
                                                                       random_state=42)
    ...
    #                                         patience=10, lr=5e-3, dropout=0.3, weight_decay_value=0, pooling='mean', 
    #                                         batch_size=64, device=device, method=method)
    
    model_dict = train_final_model(data_by_patient=data_by_patient, model_class=LSTMClassifier, hidden_dim=8, num_layers=4, max_epochs=100, 
                                            patience=10, lr=5e-3, dropout=0.3, weight_decay_value=0, pooling='mean', 
                                            batch_size=64, error_analysis=False, feature_names=feature_names, method=method, device=device,
                                            model_name=f"{dataset}_{target_len}-{pred_length}_{method}_paper")
...
# # Il file verrà salvato come LSTM_SepsisExp.gv.pdf o .png
