...
from sklearn.metrics import confusion_matrix, roc_auc_score, accuracy_score, matthews_corrcoef, brier_score_loss, classification_report, precision_score, recall_score, f1_score, accuracy_score, r2_score, mean_absolute_error, log_loss
...
def soft_label_logistic(dt, mu, s):
    """
    Funzione logistica stabile numericamente.
    dt: distanza dall'onset (in ore o timestep)
    mu: punto in cui la probabilità = 0.5
    s: parametro di scala (controlla la pendenza)
    """
    z = (dt - mu) / (s if s > 0 else 1e-6)
    # forma numericamente stabile della sigmoide
    if z >= 0:
...
def pad_or_truncate_sequence(
    ...
    if mu is None:
        mu = prediction_horizon / 2.0   # punto medio → 0.5 a metà PH
    if s is None:
        s = max(1.0, prediction_horizon / 4.0)  # pendenza ragionevole

    earliest_end = window_size-1
    latest_end = sepsis_onset - prediction_horizon

    ends = []
    e = latest_end
    while e>=earliest_end:
        ends.append(e)
        e -= prediction_step
    ...
    for e in ends:
        start_index = max(0, e - window_size + 1)
        truncated_seq = seq[start_index : e + 1]

        # Padding se troppo corta
        if len(truncated_seq) < window_size:
            pad_len = window_size - len(truncated_seq)
            padding = torch.full((pad_len, *seq.shape[1:]), padding_value)
            truncated_seq = torch.cat([padding, truncated_seq], dim=0)
        ...
        if len(truncated_seq) != window_size:
            print(" ERRORE: lunghezza finale anomala")
            print(f"  onset={sepsis_onset}, start={start_index}, end={e}")
            print(f"  truncated shape: {truncated_seq.shape}")
        
        windows.append(truncated_seq)

        dt = sepsis_onset - e
        prob = soft_label_logistic(dt, mu=mu, s=s)
        labels.append(prob)
    
    return windows, [1,2,3,4]
    

class LSTMClassifier(nn.Module):
    def __init__(self, input_dim, hidden_dim=64, num_layers=2, bidirectional=True, dropout=0.3, pooling="mean", num_classes=5):
        super().__init__()
...
def train_and_evaluate_kfold_val_earlystop(
    ...
    patient_ids = list(data_by_patient.keys())
    patient_labels = [data_by_patient[pid]["label"].item() for pid in patient_ids]
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(patient_ids, patient_labels)):
        ...
        print(f"N. of train ids: {len(train_ids)} - N. of test ids: {len(test_ids)}")

        train_windows, train_labels = [], []
        test_windows, test_labels = [], []

        for pid in train_ids:
            if data_by_patient[pid]['label']==1:
                w1, w2, w3, w4 = data_by_patient[pid]["x"]
                l1, l2, l3, l4 = data_by_patient[pid]["y"]
                train_windows.extend((w1, w2, w3, w4))
                train_labels.extend((l1, l2, l3, l4))
            
            else:
                w1 = data_by_patient[pid]["x"]
                l1 = data_by_patient[pid]["y"]
                train_windows.extend((w1))
                train_labels.extend((l1))

        for pid in test_ids:
            if data_by_patient[pid]['label']==1:
                w1, w2, w3, w4 = data_by_patient[pid]["x"]
                l1, l2, l3, l4 = data_by_patient[pid]["y"]
                test_windows.extend((w1, w2, w3, w4))
                test_labels.extend((l1, l2, l3, l4))
            
            else:
                w1 = data_by_patient[pid]["x"]
                l1 = data_by_patient[pid]["y"]
                test_windows.extend((w1))
                test_labels.extend((l1))

        X_train_full = torch.stack(train_windows)
        y_train_full = torch.stack(train_labels)
        X_test = torch.stack(test_windows)
        y_test = torch.stack(test_labels)
        ...
        # # --- CALCOLO DEL PESO DELLA CLASSE (CLASS WEIGHTING) ---
        # # 1. Calcola il conteggio delle etichette (solo sul training set interno)
        # y_train_counts = torch.count_nonzero(y_train_full[train_dataset.indices])
        # n_samples = len(train_dataset)
        ...
        # Aggiungi chiavi per ogni classe
        for i in range(n_classes):
            fold_metrics[f"Precision_{i}"] = []
            fold_metrics[f"Recall_{i}"] = []
            fold_metrics[f"F1_{i}"] = []
        ...
        plt.plot(val_loss_trend, label="Val Loss")
        plt.legend()
        plt.title(f"Train vs Val Loss (fold {fold+1})")
        plt.savefig(f"./model_results/loss_trend_fold-{fold+1}.png", dpi=300)
        plt.clf()
        ...
        with torch.no_grad():
            logits = model(X_test.to(device))
            probs = torch.softmax(logits, dim=1)   # ✅ Softmax per probabilità
       ...
       # ---------------- TEST FINALE ----------------
        ...
        with torch.no_grad():
            logits = model(X_test.to(device))
            probs = torch.softmax(logits, dim=1)   # ✅ Softmax su tutte le classi
            preds = torch.argmax(probs, dim=1).cpu().numpy()
            y_true = y_test.cpu().numpy()
            y_prob = probs.cpu().numpy()            # ✅ tutte le colonne (n_classi)
        ...
        try:
            ...
            auc = float("nan")  # in caso una classe manchi nel fold
        ...
        print(f"Test AUC (weighted, OVR) = {auc:.3f}")
        ...
        # ---------------- SALVATAGGIO METRICHE PER FOLD ----------------
        fold_metrics["ACC"].append(acc)
        fold_metrics["AUC"].append(auc)
        fold_metrics["MCC"].append(mcc)
        fold_metrics["Log loss"].append(logloss)

        for i in range(len(np.unique(y_true))):
            precision_i = report[str(i)]["precision"]
            recall_i = report[str(i)]["recall"]
            f1_i = report[str(i)]["f1-score"]
            fold_metrics[f"Precision_{i}"].append(precision_i)
            fold_metrics[f"Recall_{i}"].append(recall_i)
            fold_metrics[f"F1_{i}"].append(f1_i)
    ...
    # print("\nRisultati Medi:")
    # print(f"Avg. R2: {np.mean(r2_results)} +/- {np.std(r2_results)}")
    # print(f"Avg. MAE: {np.mean(mae_results)} +/- {np.std(mae_results)}")
    ...
    # ======================================================
    # Dopo il ciclo KFold: stampa risultati medi
    # ======================================================
    print("\n================= RISULTATI MEDI =================")
    print(f"ACC={np.mean(fold_metrics['ACC']):.3f} | "
        f"AUC={np.mean(fold_metrics['AUC']):.3f} | "
        f"MCC={np.mean(fold_metrics['MCC']):.3f} | "
        f"Log loss={np.mean(fold_metrics['Log loss']):.3f}")

    print("Classe 0 (non sepsis): "
        f"P={np.mean(fold_metrics['Precision_0']):.3f}, "
        f"R={np.mean(fold_metrics['Recall_0']):.3f}, "
        f"F1={np.mean(fold_metrics['F1_0']):.3f}")

    print("Classe 1 (sepsis): "
        f"P={np.mean(fold_metrics['Precision_1']):.3f}, "
        f"R={np.mean(fold_metrics['Recall_1']):.3f}, "
        f"F1={np.mean(fold_metrics['F1_1']):.3f}")
    print("==================================================")
...
def prepare_mimic_data(
    ...
    for patient_id, group in mimic_df.groupby("Index"):
        ...
        if y == 1:
            ...
            patient_windows, soft_labels = pad_or_truncate_sequence(
                x_tensor,
                sepsis_onset=sepsis_onset_index,
            ...
            earliest_end = window_size-1
            latest_end = sepsis_onset_index - prediction_horizon

            random_end_index = randint(earliest_end, latest_end)
            start_index = max(0, random_end_index - window_size + 1)
            patient_windows = [x_tensor[start_index : random_end_index + 1]]
            soft_labels = [0]
            
            # print(f"Patient: {patient_id}; Label: {soft_labels}")
            # print(f"Start index: {start_index} - End index: {random_end_index}")
            # print(f"Shape: {patient_windows[0].shape}")


        data_by_patient[patient_id] = {
                "x": patient_windows,
                "y": torch.tensor(soft_labels, dtype=torch.long),
                "label": y
            }
        ...
        #     print(f"\n🧪 Paziente {patient_id}")
        #     print(f"Totale finestre generate: {len(patient_windows)}")
        #     print(f"Sequenza originale shape: {x_tensor.shape}\n")

        #     for i, (w, lbl) in enumerate(zip(patient_windows, soft_labels)):
        #         # Calcola gli indici effettivi usati per estrarre questa finestra
        #         e = sepsis_onset_index - prediction_horizon - (len(patient_windows) - 1 - i) * prediction_step
        #         start_index = max(0, e - window_size + 1)

        #         print(f"  🪟 Finestra {i+1}: start={start_index}, end={e}, shape={w.shape}, label={lbl:.3f}")
        #         print(f"    Ultimi 3 timestep (dalla finestra):\n{w[-3:, :5]}")
        #         print(f"    Ultimi 3 timestep (dall'originale x_tensor):\n{x_tensor[e-2:e+1, :5]}\n")

        #         # Controlla se coincidono (entro piccole tolleranze numeriche)
        #         if torch.allclose(w[-3:, :5], x_tensor[e-2:e+1, :5], atol=1e-5):
        #             print("    ✅ Match perfetto tra finestra e dati originali.\n")
        #         else:
        #             print("    ⚠️  Differenze rilevate! Possibile errore di slicing o padding.\n")  
...
def train_final_model(
    ...
    # criterion = FocalLoss(alpha=1.0, gamma=1.0, reduction="mean")
    optimizer = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay_value)
...
if __name__=="__main__":
    ...
    if dataset=="mimic":
        data_by_patient, device, _, _, _, _= prepare_mimic_data(window_size=window_size, prediction_horizon=prediction_horizon, prediction_step=prediction_step,
        features_selected=features_selected, pop_patient=False, n=1, random_state=42)
    ...
    train_and_evaluate_kfold_val_earlystop(data_by_patient=data_by_patient, model_class=LSTMClassifier, 
        hidden_dim=8, num_layers=4, pooling='mean', dropout=0.3, lr=1e-3, weight_decay_value=1e-4, batch_size=64,
        max_epochs=400, patience=50, device=device, method=method)
    
    # model_dict = train_final_model(data_by_patient=data_by_patient, model_class=LSTMClassifier, method=method, 
    # hidden_dim=4, num_layers=2, pooling='mean', dropout=0.3, lr=5e-3, weight_decay_value=1e-4, batch_size=64, device=device, 
    # max_epochs=200, patience=10, error_analysis=False, feature_names=feature_names, model_name=f"{dataset}_{target_len}-{pred_length}_{method}")
