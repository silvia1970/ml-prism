...
def analyze_csvs(folder_path, verbose=True):
    ...
    if verbose:
        ...
        iterator = tqdm(csv_files, desc="CSVs composition for SepsisExp", unit="file")
...
def prepare_sepsisexp_data(
    ...
    _, merged_df, _ = analyze_csvs(folder_path=sepsisexp_path, verbose=False)
    ...
    if verbose:
        print(f"    Df shape: {hour_df.shape}")
    ...
    return data_by_patient, device, hour_df
...
def balance_patients(data_by_patient, method="oversample", random_state=42):
    ...
    print(f"    (Pre balancing) Septic patients: {n_septic}, Non septic patients: {n_non_septic}")
...
"""Feature Selection"""
def select_features_lasso_from_timeseries(hour_df: pd.DataFrame, C_value: float = 0.1) -> tuple[pd.DataFrame, pd.Series]:
    """
    Esegue l'estrazione delle feature da serie temporali tramite aggregazione statistica 
    (chunking) e poi applica la Feature Selection con L1 Regularization (Lasso).

    Args:
        hour_df: DataFrame delle serie temporali, deve contenere le colonne 'id', 
                 'sepsis', 'timestep', e le colonne delle feature cliniche.
        C_value: Iperparametro C per Lasso (inverso della regolarizzazione). 
                 Valori più piccoli = selezione più aggressiva.

    Returns:
        Una tupla contenente: 
        1. X_reduced_tabular: DataFrame con solo le feature selezionate, indicizzato per 'id'.
        2. y_fs: Series del target 'sepsis', indicizzata per 'id'.
    """

    # --- FASE 1: Estrazione Feature Aggregata (Chunking Ottimizzato) ---
    aggregated_data = []
    target_y = []

    # 1. Preparazione dell'iteratore raggruppato
    grouped_patients = hour_df.groupby("id")
    total_patients = len(grouped_patients)

    print("\n--- FASE 1/2: Estrazione Statistiche (Chunking) Ottimizzata ---")

    for patient_id, group in tqdm(grouped_patients, total=total_patients, desc="Pazienti Elaborati"):
        
        # Preparazione dei dati e del target
        # Rimozione di 'id', 'timestep', 'severity' (come richiesto) e 'sepsis'
        x = group.drop(columns=["id", "sepsis", "timestep", "severity"]) 
        y = group["sepsis"].iloc[0]
        
        patient_features = {'id': patient_id}
        target_y.append({'id': patient_id, 'sepsis': y})

        # Calcolo Statistiche Standard (una sola volta con .describe())
        x_statistics = x.describe(include='all')

        for feature_name in x_statistics.columns:
            stats = x_statistics[feature_name]
            series = x[feature_name]

            # Statistiche Standard
            patient_features[f'{feature_name}_mean'] = stats.loc['mean']
            patient_features[f'{feature_name}_std'] = stats.loc['std']
            patient_features[f'{feature_name}_min'] = stats.loc['min']
            patient_features[f'{feature_name}_max'] = stats.loc['max']
            patient_features[f'{feature_name}_25p'] = stats.loc['25%']
            patient_features[f'{feature_name}_median'] = stats.loc['50%']
            patient_features[f'{feature_name}_75p'] = stats.loc['75%']

            # Statistiche Aggiuntive
            patient_features[f'{feature_name}_var'] = series.var()
            patient_features[f'{feature_name}_range'] = series.max() - series.min()
            patient_features[f'{feature_name}_skew'] = series.skew()
            patient_features[f'{feature_name}_kurt'] = series.kurt()
            
            # Moda (gestione di possibili NaN se la serie è vuota)
            try:
                patient_features[f'{feature_name}_mode'] = series.mode().iloc[0]
            except IndexError:
                patient_features[f'{feature_name}_mode'] = float('nan')
            
        aggregated_data.append(patient_features)

    # Creazione Efficiente del DataFrame Finale
    X_tabular = pd.DataFrame(aggregated_data).set_index('id')
    y_fs = pd.DataFrame(target_y).set_index('id')['sepsis']
    
    # Gestione NaN residui (necessario prima di Lasso)
    X_tabular = X_tabular.fillna(X_tabular.mean()) # Imputazione con la media
    
    print("--- Estrazione Completata ---")
    print(f"Dimensione X_tabular (Originale Aggregata): {X_tabular.shape}")
    
    # --- FASE 2: Feature Selection con Lasso (L1) ---
    print("\n--- FASE 2/2: Feature Selection con Lasso (L1) ---")

    # 1. Addestramento del Modello con L1 (Assumiamo che X_tabular sia già Z-score normalizzato)
    ...
    lasso_model.fit(X_tabular, y_fs)

    # 2. Estrazione dei Coefficienti e Selezione delle Feature
    ...
    X_reduced_array = model_selector.transform(X_tabular)
    selected_features_indices = model_selector.get_support(indices=True)
    selected_feature_names = X_tabular.columns[selected_features_indices]

    # 3. Creazione del DataFrame Finale
    X_reduced_tabular = pd.DataFrame(
        X_reduced_array, 
    ...
    # Output
    print("\n--- Risultati della Feature Selection ---")
    print(f"Iperparametro di Regolarizzazione (C): {C_value}")
    print(f"Numero di feature originali: {X_tabular.shape[1]}")
    print(f"Numero di feature mantenute: {X_reduced_tabular.shape[1]}")
    print("\nFeature Mantenute (e loro peso stimato):")
    
    output_info = []
    for name in selected_feature_names:
        idx = np.where(X_tabular.columns == name)[0][0]
        output_info.append(f"- {name}: (Coef: {coefs[idx]:.4f})")
    
    # Stampa i primi 30 risultati per brevità
    print('\n'.join(output_info[:30]) + ('\n...' if len(output_info) > 30 else ''))
    
    return X_reduced_tabular, y_fs
...
# Definisci la tua classe modello (assumiamo sia importata o definita come sopra)
# class LSTMClassifier(nn.Module): ...
...
if __name__=="__main__":
    ...
    #original features
    features_selected = ['c-reactive_protein', 'respiratory_rate', 'heart_time_volume', 'bilirubin', 'blood_urea_nitrogen', 'age', 'temperature', 
    'calcium', 'chloride', 'alanine_transaminase', 'net balance', 'blood_glucose', 'respiratory_minute_volume', 'mean_bp', 'svri', 'quick_score', 
    'stroke_volume', 'bicarbonate', 'aspartate_transaminase', 'creatinine', 'base_excess', 'procalcitonin', 'sodium', 'lymphocytes', 'oxygen_saturation',
     'bun/creatinine_ratio', 'diastolic_bp', 'potassium', 'delta-temperature', 'lactate', 'urine_output', 'arterial_ph', 'hemoglobin', 
     'fraction_of_inspired_o2', 'mixed_venous_oxygen_saturation', 'horowitz_index', 'pancreatic_lipase', 'partial_co2', 'systolic_bp', 
     'partial_pressure_art._o2', 'heart_rate', 'leukocytes', 'thrombocytes']

    features_selected = ['id', 'sepsis', 'severity', 'timestep'] + features_selected 

    if dataset=="sepsisexp":
        data_by_patient, device, hour_df = prepare_sepsisexp_data(selected_features=features_selected, target_len=target_len, pred_length=pred_length, 
                                                                     pop_patient=False, n=2, random_state=42)
        
        feature_names = hour_df.drop(columns=["id", "sepsis", "timestep", "severity"]).columns.to_list()

    else:
        raise ValueError(f"Dataset not recognized: '{dataset}'. Insert a valid name ('sepsisexp' or 'mimic').")
    

    train_and_evaluate_kfold_val_earlystop(data_by_patient=data_by_patient, model_class=LSTMClassifier, 
        hidden_dim=2, num_layers=4, pooling='max', dropout=0.2, lr=1e-3, weight_decay_value=0.8e-4, batch_size=64,
        max_epochs=200, patience=10, device=device, method=method)
    
    model_dict = train_final_model(data_by_patient=data_by_patient, model_class=LSTMClassifier, method=method, 
        hidden_dim=2, num_layers=4, pooling='max', dropout=0.2, lr=1e-3, weight_decay_value=0.8e-4, batch_size=64, 
        device=device, max_epochs=200, patience=10, error_analysis=False, feature_names=feature_names,
         model_name=f"{dataset}_{target_len}-{pred_length}_{method}")

    # 1. Definisci la Griglia di Parametri da Esplorare
    # param_grid_full = {
    # # 1. PARAMETRI DI ARCHITETTURA (Architettura della LSTM)
    # 'hidden_dim': [8, 16, 64],  # Dimensione del vettore nascosto LSTM
    # 'num_layers': [4, 8, 16],      # Numero di strati LSTM (profondità)
    # 'pooling': ['mean', 'last', 'max'],  # Metodo di pooling (l'abbiamo fissato a 'mean' ma si può testare)
    
    # # 2. PARAMETRI DI REGOLARIZZAZIONE (Regularization)
    # 'dropout': [0.3, 0.4, 0.5], # Il tuo 0.3 ottimale e alternative più aggressive
    
    # # 3. PARAMETRI DI ADDESTRAMENTO (Optimization)
    # 'lr': [1e-3, 5e-3, 1e-4],          # Learning Rate (i due valori che stavi testando)
    # 'weight_decay_value': [1e-3, 5e-3, 1e-4], # Valore L2 (il tuo 1e-4 ottimale + alternative)
    # 'batch_size': [16, 32, 64]
    # }
    ...
    # selected_feature_names = automatic_pfi_feature_selection(importance_df, positive_threshold=0.05)
    ...
    # error_analysis_df.to_csv("./error_analysis/error_analysis.csv")

    # prob_stats = model_dict['error_group_stats']['probability_stats']
    # prob_stats.to_csv("./error_analysis/prob_stats.csv")
    ...
    # feature_df.to_csv("./error_analysis/feature_comp_per_group.csv")
