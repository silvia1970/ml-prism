...
def has_valid_preonset_window(group, target_len, pred_length):
    onset = np.argmax(group["severity"].values)
    return onset - pred_length > 0
...
def train_and_evaluate_kfold_val_earlystop(
    ...
    data=data[['id','timestep', 'severity', 'sepsis', 'age', 'heart_rate', 'svri', 'mean_bp', 'heart_time_volume', 'oxygen_saturation', 
    'delta-temperature', 'mixed_venous_oxygen_saturation', 'lactate', 'creatinine', 'bilirubin', 'sodium', 'potassium', 'hemoglobin',
    'chloride', 'leukocytes', 'bicarbonate', 'pancreatic_lipase', 'blood_urea_nitrogen', 'procalcitonin', 'bun/creatinine_ratio', 'aspartate_transaminase',
    'c-reactive_protein', 'respiratory_minute_volume', 'fraction_of_inspired_o2', 'arterial_ph', 'partial_pressure_art._o2']]

    measurement_cols = data.drop(columns=['id', 'timestep', 'severity', 'sepsis']).columns
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(patient_ids, patient_labels)):
        ...
        train_df = data[data[id_col].isin(train_ids)]
        test_df  = data[data[id_col].isin(test_ids)]
        ...
        print()
        train_df, test_df = normalize_df(train_df, test_df, measurement_cols, save_path=f'datasets/SepsisExp/original_sepsisexp/sepsisexp_normalization_global_{fold+1}.json')

        valid_train_ids = []
        for pid, g in train_df.groupby("id"):
            if has_valid_preonset_window(g, target_len, prediction_horizon):
                valid_train_ids.append(pid)

        train_min_len_df = train_df[train_df["id"].isin(valid_train_ids)]

        valid_test_ids = []
        for pid, g in test_df.groupby("id"):
            if has_valid_preonset_window(g, target_len, prediction_horizon):
                valid_test_ids.append(pid)

        test_min_len_df = test_df[test_df["id"].isin(valid_test_ids)]
        ...
        train_data_by_patient = prepare_sepsisexp_data(train_min_len_df, target_len=target_len, pred_length=prediction_horizon, desc="Prepare (padding, cut, truncate, ecc.) training data")
        test_data_by_patient = prepare_sepsisexp_data(test_min_len_df, target_len=target_len, pred_length=prediction_horizon,  desc="Prepare (padding, cut, truncate, ecc.) test data")
...
def prepare_sepsisexp_data(
    ...
    if pop_patient:
        ...
        unique_ids = sepsisexp_df['id'].unique()
        
        # 1. Trova l'etichetta 'sepsis' (y) per ogni paziente
        patient_labels = sepsisexp_df.groupby('id')['sepsis'].first()
        ...
        subset_df = sepsisexp_df[sepsisexp_df['id'].isin(selected_ids)].copy()
        remaining_df = sepsisexp_df[~sepsisexp_df['id'].isin(selected_ids)].copy()
    ...
    data_by_patient = {}
    for patient_id, group in tqdm(sepsisexp_df.groupby("id"), desc=desc):
    ...
    # for pid, d in data_by_patient.items():
    #     x = d["x"]
    #     lengths = (x != padding_value).any(dim=1).sum().item()
    #     if lengths == 0:
    #         zero_len_patients.append(pid)

    # print("Patients with zero-length sequences:", zero_len_patients)
...
def train_final_model(
    ...
    loss_fn=None,
    method="undersample",
    model_name='sepsisexp_final_model',
    seed=42,
    weight_decay_value=0):
    ...
    static_cols = ['sepsis']
    measurement_cols = data.columns.drop(static_cols + [id_col, 'timestep', 'severity'])
    print(measurement_cols)

    data_norm, _ = normalize_df(
        data, data, measurement_cols,
        save_path="datasets/SepsisExp/original_sepsisexp/scaler_stats_final.json"
    )

    valid_ids = []
    for pid, g in data_norm.groupby("id"):
        if has_valid_preonset_window(g, target_len, prediction_horizon):
            valid_ids.append(pid)
    train_min_len_df = data_norm[data_norm["id"].isin(valid_ids)]


    data_by_patient = prepare_sepsisexp_data(
        train_min_len_df,
...
"""NORM"""
def create_dict_sepsisexp_norm():

    part_A = pd.read_csv(
        "datasets/SepsisExp/original_sepsisexp/sepsisexp_timeseries_partition-A.tsv",
        sep="\t"
    )
    part_B = pd.read_csv(
        "datasets/SepsisExp/original_sepsisexp/sepsisexp_timeseries_partition-B.tsv",
        sep="\t"
    )
    part_C = pd.read_csv(
        "datasets/SepsisExp/original_sepsisexp/sepsisexp_timeseries_partition-C.tsv",
        sep="\t"
    )
    part_D = pd.read_csv(
        "datasets/SepsisExp/original_sepsisexp/sepsisexp_timeseries_partition-D.tsv",
        sep="\t"
    )

    # --- Merge all partitions ---
    df = pd.concat([part_A, part_B, part_C, part_D], axis=0, ignore_index=True)

    # --- Columns to exclude ---
    exclude_cols = ['id', 'sepsis', 'severity', 'timestep']
    ...
    for col in feature_cols:
        if col == 'age':
            tmp = df[['id', col]].drop_duplicates('id')[col]
        else:
            tmp = df[col]

        norm_dict[col] = {
            "mean": float(tmp.mean()),
            "std": float(tmp.std(ddof=0))
        }
...
if __name__=="__main__":
    ...
    my_sepsisexp_df = create_dict_sepsisexp_norm()
    exlcude_cols = ['id', 'sepsis', 'severity', 'timestep']

    feature_names = my_sepsisexp_df.drop(columns=exlcude_cols).columns.tolist()
    print(feature_names)

    print(f"Len. features: {len(feature_names)}")

    train_and_evaluate_kfold_val_earlystop(data=my_sepsisexp_df, min_length=min_input_len, prediction_horizon=pred_length, target_len = target_len, 
    ...
    # model_dict = train_final_model(data=my_sepsisexp_df, min_length=min_input_len, prediction_horizon=pred_length, target_len = target_len, 
    #                                         model_class=LSTMClassifier, hidden_dim=8, num_layers=4, max_epochs=100, patience=10, lr=5e-3, dropout=0.3, 
    #                                         weight_decay_value=0, pooling='mean', batch_size=64, device=device, method=method, 
    #                                         model_name=f"sepsisexp_{target_len}-{pred_length}_{method}_29features_norm")
